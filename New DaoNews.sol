...
function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32
...
contract daonews is Context, IERC20, Ownable {
using SafeMath for uint256;
using Address for address;
address payable public marketingWalletAddress =
payable(0xF6a610480f78C24D9dC7a0Bec945fE303e55B533); // Marketing Wallet Address
address payable public LiquidityWalletAddress =
payable(0x0448d32668912C3a72D4daEB3dD8231f9f1E2e23); // Liquidity Wallet Address
address payable public CharityWalletAddress =
payable(0x3802154743A969F039E3bEFeE939e60eb4F032f4); // Charity Wallet Address
...
string private _name = "dao-news";
string private _symbol = "NWS";
uint8 private _decimals = 9;
uint256 public _burnFee = 0; //0% Burn
uint256 public _liquidityFee = 250; //2.5% Goes to Liquidity Wallet from every transaction
uint256 public _marketingFee = 250; //2.5% Goes to Marketing Wallet from every transaction
uint256 public _CharityFee = 0; //0% Charity
uint256 public feeDenominator = 10000;
uint256 public _totalTaxPercent = 0; //Showing 0 Here , But it is Set to 10 % overall as Discussed.
...
IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Prod
uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
.createPair(address(this), _uniswapV2Router.WETH());
uniswapV2Router = _uniswapV2Router;
_isExcludedFromFee[owner()] = true;
_isExcludedFromFee[address(this)] = true;
_totalTaxPercent = _burnFee.add(_liquidityFee).add(_marketingFee).add(_CharityFee);
...
function reflectionFromToken(uint256 tAmount, bool deductTransferFee) private view
returns(uint256) {
...
if (overMinimumTokenBalance && !inSwapAndLiquify && from != uniswapV2Pair &&
...
newBalance.div(_marketingFee.add(_CharityFee)).mul(_marketingFee);
uint256 WalletShare = newBalance.sub(marketingShare);
if(marketingShare > 0)
transferToAddressETH(marketingWalletAddress, marketingShare);
if(WalletShare > 0)
transferToAddressETH(CharityWalletAddress, WalletShare);
}
if(forBurn > 0)
{
...
(uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256
tLiquidity) = _getValues(tAmount);
_rOwned[sender] = _rOwned[sender].sub(rAmount);
...
(uint256 rAmount, uint256 rTransferAmount) = _getRValues(tAmount, tLiquidity,
_getRate());
...
function _getRValues(uint256 tAmount, uint256 tLiquidity, uint256 currentRate) private
...
function setTaxes(uint256 newBurnFee, uint256 newLiquidityTax, uint256
newMarketingTax, uint256 newCharityTax, uint256 _feeDenominator) external onlyOwner() {
_burnFee = newBurnFee;
